...
function buildHRLink(applicant, results) {
  const payload = { a: applicant, r: { overall: results.overall, recommendation: results.recommendation, timeTaken: results.timeTaken, totalRedFlags: results.totalRedFlags, traitResults: results.traitResults.map(t=>({id:t.trait.id,name:t.trait.name,abbr:t.trait.abbr,pct:t.pct,redFlags:t.redFlags})), inconsistencies: results.inconsistencies, interviewQs: results.interviewQs } };
...
function parseHRLink() {
  ...
  if (!hr) return null;
  try { return JSON.parse(decodeURIComponent(escape(atob(hr)))); } catch { return null; }
}
...
function saveCode(code,record){const codes=getCodes();codes[code.toUpperCase()]=record;localStorage.setItem(CODES_KEY,JSON.stringify(codes));}
function getCode(code){return getCodes()[code.toUpperCase()]||null;}
function incrementUsage(code){const codes=getCodes();const rec=codes[code.toUpperCase()];if(!rec)return;rec.used+=1;rec.lastUsed=new Date().toISOString();localStorage.setItem(CODES_KEY,JSON.stringify(codes));}
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 16px"}}>
        <div style={{width:"100%",maxWidth:560,textAlign:"center"}}>
          ...
          <p style={{fontSize:15,color:C.textMuted,margin:"0 0 36px",lineHeight:1.7}}><strong>{propertyName}</strong> has used all {FREE_LIMIT} free assessments. Choose how you'd like to continue.</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:28}}>
            <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:16,padding:"28px 24px",boxShadow:C.shadowMd}}>
              ...
              <div style={{fontSize:13,color:C.textMid,lineHeight:1.7,marginBottom:20}}>Pay as you go. No monthly fee.</div>
              <a href="mailto:info@casinoprosolutions.com" style={{display:"block",background:C.gold,color:C.white,border:"none",borderRadius:8,padding:"11px",fontSize:13,fontWeight:700,cursor:"pointer",textDecoration:"none",letterSpacing:0.5,textAlign:"center"}}>Contact Us to Activate</a>
            </div>
            <div style={{background:C.white,border:`2px solid ${C.gold}`,borderRadius:16,padding:"28px 24px",boxShadow:`0 0 24px rgba(184,134,11,0.15)`,position:"relative"}}>
              ...
              <div style={{fontSize:13,color:C.textMid,lineHeight:1.7,marginBottom:20}}>Unlimited assessments forever.</div>
              <a href={GUMROAD_URL} target="_blank" rel="noreferrer" style={{display:"block",background:C.gold,color:C.white,borderRadius:8,padding:"11px",fontSize:13,fontWeight:700,cursor:"pointer",textDecoration:"none",letterSpacing:0.5,textAlign:"center"}}>Purchase on Gumroad →</a>
            </div>
          </div>
        </div>
      </div>
...
function WarningBanner({remaining}){
  return(
    <div style={{background:C.amberBg,border:`1px solid ${C.amberBorder}`,borderRadius:10,padding:"12px 18px",marginBottom:20,display:"flex",gap:12,alignItems:"center"}}>
      ...
      <div>
        <div style={{fontSize:13,fontWeight:700,color:C.amber}}>{remaining} Free Assessment{remaining!==1?"s":""} Remaining</div>
        <div style={{fontSize:12,color:C.textMuted,marginTop:2}}>After your free trial ends, assessments are $15 each or $297 for unlimited access.</div>
      </div>
    </div>
  );
}
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 16px"}}>
        <div style={{width:"100%",maxWidth:540}}>
          <div style={{background:C.goldBg,border:`1px solid ${C.goldBorder}`,borderRadius:10,padding:"12px 20px",marginBottom:28,display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:36,height:36,borderRadius:8,background:C.white,border:`1px solid ${C.goldBorder}`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Playfair Display',serif",fontWeight:700,color:C.gold,fontSize:16}}>{applicant.name.charAt(0)}</div>
            <div>
          ...
          <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:C.shadowLg,padding:"36px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:32}}>
              {[["⏱","7-minute timed assessment","The countdown begins the moment you click Begin."],["➡️","One question at a time — forward only","Each question appears on its own screen. Once you select an answer and advance, you cannot return to a previous question."],["📊","Frequency scale","Rate each statement: Never · Rarely · Sometimes · Often · Always"],["🔒","Answer independently","Treat each question on its own merit. Respond honestly to each one."]].map(([icon,title,desc])=>(
            ...
            <div style={{background:C.amberBg,border:`1px solid ${C.amberBorder}`,borderRadius:10,padding:"12px 16px",marginBottom:24,display:"flex",gap:10,alignItems:"flex-start"}}>
              ...
              <span style={{fontSize:13,color:C.amber,fontWeight:500,lineHeight:1.5}}><strong>No back button.</strong> Once you move to the next question, your answer is final.</span>
            </div>
...
function ThankYou({applicant,results,hrLink,onViewReport}){
  ...
  function downloadPdf(){
    ...
    generateHRPdf(applicant,results).then(name=>{setFilename(name);setPdfStatus("done");}).catch(()=>setPdfStatus("error"));
  }
  const [copied,setCopied]=useState(false);
  function copyLink(){navigator.clipboard.writeText(hrLink).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2500);});}
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      <TopBar/>
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 16px"}}>
        <div style={{width:"100%",maxWidth:520,textAlign:"center"}}>
          ...
          <h1 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:30,fontWeight:700,color:C.text,margin:"0 0 12px"}}>Assessment Complete</h1>
          <p style={{fontSize:16,color:C.textMid,margin:"0 0 32px",lineHeight:1.7}}>Thank you, <strong>{applicant.name}</strong>.<br/>Your responses have been submitted successfully.</p>
          <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:16,padding:"28px 32px",boxShadow:C.shadowMd,textAlign:"left",marginBottom:20}}>
            ...
            {[["📋","Your assessment has been received","Your responses are being reviewed by the hiring team."],["🔒","Results are confidential","Assessment scores are reviewed only by the HR team."],["📞","You will be contacted","The hiring team will be in touch regarding next steps."]].map(([icon,title,desc])=>(
          ...
          <div style={{background:pdfStatus==="done"?C.greenBg:pdfStatus==="error"?C.amberBg:C.goldBg,border:`1px solid ${pdfStatus==="done"?C.greenBorder:pdfStatus==="error"?C.amberBorder:C.goldBorder}`,borderRadius:10,padding:"12px 18px",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{textAlign:"left"}}>
                <div style={{fontSize:13,fontWeight:600,color:pdfStatus==="done"?C.green:pdfStatus==="error"?C.amber:C.gold}}>
                  {pdfStatus==="done"?`Downloaded: ${filename}`:pdfStatus==="error"?"PDF unavailable — use HR link below":"Download HR Report PDF"}
                </div>
                {pdfStatus==="done"&&<div style={{fontSize:11,color:C.textMuted,marginTop:2}}>Check your Downloads folder.</div>}
              </div>
            </div>
            {pdfStatus==="ready"&&<button onClick={downloadPdf} style={{background:C.gold,color:C.white,border:"none",borderRadius:8,padding:"8px 16px",fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>Download PDF</button>}
            {pdfStatus==="generating"&&<span style={{fontSize:12,color:C.gold}}>⏳ Generating…</span>}
          </div>
          ...
          <div style={{marginTop:32,borderTop:`1px solid ${C.border}`,paddingTop:20}}>
            ...
            <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 20px",boxShadow:C.shadow,textAlign:"left"}}>
              <div style={{fontSize:13,color:C.textMuted,marginBottom:12,lineHeight:1.6}}>Share this link with the hiring manager to access the full scored report on any device:</div>
...
function HRPdfButton({applicant,results}){
  ...
  function handleDownload(){
    ...
    generateHRPdf(applicant,results)
      .then(name=>{setFilename(name);setStatus("done");})
...
function HRReport({results,timeTaken,applicant}){
  ...
  const HR_PIN="1234";
  function handleUnlock(){if(pin===HR_PIN){setUnlocked(true);setPinError("");}else{setPinError("Incorrect PIN. Please try again.");setPin("");}}
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 16px"}}>
        <div style={{width:"100%",maxWidth:420}}>
          ...
          <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:C.shadowLg,padding:"32px 36px"}}>
            ...
            <p style={{fontSize:11,color:C.textFaint,marginBottom:16}}>Demo PIN: <strong style={{color:C.textMid,letterSpacing:2}}>1234</strong></p>
          ...
          <div>
            ...
            <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 24px",boxShadow:C.shadow}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
                {[[C.green,C.greenBg,C.greenBorder,"75–100%","Strong","Consistent with the dealer role."],[C.amber,C.amberBg,C.amberBorder,"55–74%","Moderate","May need coaching in this area."],[C.red,C.redBg,C.redBorder,"0–54%","Needs Improvement","Concerns raised; further review advised."],[C.red,C.redBg,C.redBorder,"Red Flag","Critical","Potentially disqualifying responses."]].map(([col,bg,brd,range,label,desc])=>(
...
function AdminDashboard({onExit}){
  ...
  function handleUnlock(){if(pin===ADMIN_PIN){setUnlocked(true);setCodes(getCodes());}else{setPinError("Incorrect PIN.");setPin("");}}
  function handleCreateCode(){
    ...
    const record={code:newCode.code.toUpperCase(),propertyName:newCode.propertyName,contactName:newCode.contactName,email:newCode.email,plan:newCode.plan,used:0,freeLimit:FREE_LIMIT,createdAt:new Date().toISOString(),lastUsed:null};
    saveCode(newCode.code,record);setCodes(getCodes());
    setNewCode({code:"",propertyName:"",contactName:"",email:"",plan:"trial"});
    setSaved(`✅ Code ${record.code} created for ${record.propertyName}`);
    setTimeout(()=>setSaved(""),3000);
  }
  function handleUpgrade(code){const c=getCodes();c[code].plan="paid";localStorage.setItem(CODES_KEY,JSON.stringify(c));setCodes(getCodes());}
    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"32px 16px 64px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:14,marginBottom:28}}>
          {[["Total Properties",codeList.length,C.gold],["Active Trials",codeList.filter(c=>c.plan==="trial"&&c.used<FREE_LIMIT).length,C.green],["Trial Expired",codeList.filter(c=>c.plan==="trial"&&c.used>=FREE_LIMIT).length,C.red],["Paying / Purchased",codeList.filter(c=>c.plan==="paid"||c.plan==="purchased").length,C.amber],["Total Assessments",codeList.reduce((s,c)=>s+c.used,0),C.gold]].map(([label,val,color])=>(
        ...
        <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:14,padding:"24px",marginBottom:24,boxShadow:C.shadow}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            <div><label style={{fontSize:11,color:C.textMuted,fontWeight:600,letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:4}}>Property Code</label><input placeholder="e.g. ARIA2025" value={newCode.code} onChange={e=>setNewCode(n=>({...n,code:e.target.value.toUpperCase()}))} style={{...inputSt,fontFamily:"'Courier New',monospace",letterSpacing:2}}/></div>
            <div><label style={{fontSize:11,color:C.textMuted,fontWeight:600,letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:4}}>Casino / Property Name</label><input placeholder="e.g. Aria Resort & Casino" value={newCode.propertyName} onChange={e=>setNewCode(n=>({...n,propertyName:e.target.value}))} style={inputSt}/></div>
            <div><label style={{fontSize:11,color:C.textMuted,fontWeight:600,letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:4}}>Contact Name</label><input placeholder="HR Director name" value={newCode.contactName} onChange={e=>setNewCode(n=>({...n,contactName:e.target.value}))} style={inputSt}/></div>
            <div><label style={{fontSize:11,color:C.textMuted,fontWeight:600,letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:4}}>Contact Email</label><input placeholder="hr@casino.com" value={newCode.email} onChange={e=>setNewCode(n=>({...n,email:e.target.value}))} style={inputSt}/></div>
          </div>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <select value={newCode.plan} onChange={e=>setNewCode(n=>({...n,plan:e.target.value}))} style={{...inputSt,width:"auto",cursor:"pointer"}}>
              <option value="trial">Trial (10 free assessments)</option>
              <option value="paid">Paid ($15/assessment)</option>
              <option value="purchased">Purchased ($297 unlimited)</option>
            </select>
            <button onClick={handleCreateCode} style={{background:C.gold,color:C.white,border:"none",borderRadius:8,padding:"9px 24px",fontSize:13,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>Create Code</button>
          </div>
              ...
              <div key={rec.code} style={{padding:"16px 22px",borderBottom:`1px solid ${C.border}`,background:i%2===0?C.white:"rgba(0,0,0,0.01)"}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:16,flexWrap:"wrap"}}>
                  ...
                  <div style={{minWidth:120}}>
                    {!isPaid&&(<><div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textMuted,marginBottom:3}}><span>Usage</span><span style={{fontWeight:700,color:statusColor}}>{rec.used}/{FREE_LIMIT}</span></div><div style={{background:C.bg,borderRadius:99,height:5,overflow:"hidden"}}><div style={{background:statusColor,width:`${pct}%`,height:"100%",borderRadius:99}}/></div></>)}
                  ...
                  <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
                    ...
                    {rec.plan==="trial"&&rec.used>=FREE_LIMIT&&(<button onClick={()=>handleUpgrade(rec.code)} style={{background:C.gold,color:C.white,border:"none",borderRadius:6,padding:"4px 12px",fontSize:11,fontWeight:700,cursor:"pointer"}}>Upgrade</button>)}
                  </div>
                </div>
              </div>
            );
...
export default function App(){
  ...
  function handleLogoClick(){const next=adminClicks+1;setAdminClicks(next);if(next>=5){setShowAdmin(true);setAdminClicks(0);}}

  function handleCodeVerified(rec,code){
    const fresh=getCode(code)||rec;
    setProperty(fresh);
    if(fresh.plan==="trial"&&fresh.used>=FREE_LIMIT){setPhase("upgrade");}
    ...
    <div>
      {showWarning&&(phase==="welcome"||phase==="intro")&&(
        <div style={{position:"fixed",top:56,left:0,right:0,zIndex:40,padding:"0 16px"}}>
          <div style={{maxWidth:600,margin:"8px auto 0"}}><WarningBanner remaining={remaining}/></div>
        </div>
      )}
